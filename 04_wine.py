#!/usr/bin/env python
# Created by "Thieu" at 08:39, 25/04/2025 ----------%                                                                               
...
from src.data_utils import get_wine
...
if __name__ == "__main__":
    ...
    for seed in cf.LIST_SEEDS:
        ...
        rfc = RandomForestClassifier(max_depth=3, n_estimators=20, random_state=seed)
        gbc = GradientBoostingClassifier(n_estimators=20, learning_rate=0.5, max_depth=3, random_state=seed)
        mlp = MLPClassifier(alpha=0.01, max_iter=200, hidden_layer_sizes=(7,), activation="relu", random_state=seed)
    ...
    for seed in cf.LIST_SEEDS:
        md1 = AnfisClassifier(num_rules=15, mf_class="Gaussian",
                              vanishing_strategy=cf.DATA04['vanishing_strategy'], act_output=None,
                              reg_lambda=None,
                              epochs=300, batch_size=16, optim="SGD", optim_params=None,
    ...
    for opt in cf.LIST_GD_MODELS:
        for seed in cf.LIST_SEEDS:
            md2 = GdAnfisClassifier(num_rules=15, mf_class="Gaussian",
                                    vanishing_strategy=cf.DATA04['vanishing_strategy'], act_output=None,
                                    reg_lambda=None,
    ...
    for opt in cf.LIST_BIO_MODELS:
        for seed in cf.LIST_SEEDS:
            md3 = BioAnfisClassifier(num_rules=cf.DATA04['n_rules'], mf_class=cf.DATA04['mf_class'],
                                     vanishing_strategy=cf.DATA04['vanishing_strategy'], act_output=None,
                                     reg_lambda=None,
    ...
    df_loss.to_csv(f"{cf.PATH_SAVE}/{cf.DATA04['name']}/{cf.FILE_LOSS}", index=False, header=True)
    df_result.to_csv(f"{cf.PATH_SAVE}/{cf.DATA04['name']}/{cf.FILE_RESULT}", index=False, header=True)

    # Get metrics statistics
    stat_df = df_result.groupby("model_name").agg(["mean", "std"])
    stat_df.to_csv(f"{cf.PATH_SAVE}/{cf.DATA04['name']}/{cf.FILE_RESULT_STATS}", index=True, header=True)

    # Draw boxplot metrics, path_save, figsize=(8, 6), exts=(".png", ), verbose=False
    draw_boxplot(df_result, data_name=cf.DATA04['name'], list_models=cf.LIST_MODEL_CC,
                 metrics=cf.LIST_METRIC_CLS, path_save=f"{cf.PATH_SAVE}/{cf.DATA04['name']}/bp",
                 figsize=(8, 6), exts=(".png", ".pdf"), verbose=False)

    # Draw convergence chart
    draw_convergence_chart(df_loss, data_name=cf.DATA04['name'], list_models=cf.LIST_MODEL_CC,
                           path_save=f"{cf.PATH_SAVE}/{cf.DATA04['name']}/cc",
                           figsize=(8, 6), exts=(".png", ".pdf"), verbose=False)

    print(f"Done with data: {cf.DATA04['name']}")
