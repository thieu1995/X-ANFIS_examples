#!/usr/bin/env python
# Created by "Thieu" at 11:00, 25/04/2025 ----------%                                                                               
...
if __name__ == "__main__":
    ...
    for opt in cf.LIST_BIO_MODELS:
        for seed in cf.LIST_SEEDS:
            md3 = BioAnfisRegressor(num_rules=cf.DATA05['n_rules'], mf_class=cf.DATA05['mf_class'],
                                    vanishing_strategy=cf.DATA05['vanishing_strategy'],
                                    act_output=None, reg_lambda=None,
    ...
    df_loss.to_csv(f"{cf.PATH_SAVE}/{cf.DATA05['name']}/{cf.FILE_LOSS}", index=False, header=True)
    df_result.to_csv(f"{cf.PATH_SAVE}/{cf.DATA05['name']}/{cf.FILE_RESULT}", index=False, header=True)

    # Get metrics statistics
    stat_df = df_result.groupby("model_name").agg(["mean", "std"])
    stat_df.to_csv(f"{cf.PATH_SAVE}/{cf.DATA05['name']}/{cf.FILE_RESULT_STATS}", index=True, header=True)

    # Draw boxplot metrics, path_save, figsize=(8, 6), exts=(".png", ), verbose=False
    draw_boxplot(df_result, data_name=cf.DATA05['name'], list_models=cf.LIST_MODEL_CC,
                 metrics=cf.LIST_METRIC_REG, path_save=f"{cf.PATH_SAVE}/{cf.DATA05['name']}/bp",
                 figsize=(8, 6), exts=(".png", ".pdf"), verbose=False)

    # Draw convergence chart
    draw_convergence_chart(df_loss, data_name=cf.DATA05['name'], list_models=cf.LIST_MODEL_CC,
                           path_save=f"{cf.PATH_SAVE}/{cf.DATA05['name']}/cc",
                           figsize=(8, 6), exts=(".png", ".pdf"), verbose=False)

    print(f"Done with data: {cf.DATA05['name']}")
