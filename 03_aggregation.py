#!/usr/bin/env python
# Created by "Thieu" at 18:29, 24/04/2025 ----------%                                                                               
...
from src.data_utils import get_aggregation
...
if __name__ == "__main__":
    ...
    X_train, X_test, y_train, y_test = get_aggregation(path=f"{cf.PATH_READ}/{cf.DATA03['name']}.csv", verbose=False)
    ## Scaling dataset
    ...
    for seed in cf.LIST_SEEDS:
        ...
        rfc = RandomForestClassifier(max_depth=3, n_estimators=30, random_state=seed)
        gbc = GradientBoostingClassifier(n_estimators=30, learning_rate=0.5, max_depth=3, random_state=seed)
        mlp = MLPClassifier(alpha=0.01, max_iter=100, hidden_layer_sizes=(15,), activation="relu", random_state=seed)
    ...
    for seed in cf.LIST_SEEDS:
        md1 = AnfisClassifier(num_rules=cf.DATA03['n_rules'], mf_class=cf.DATA03['mf_class'],
                              vanishing_strategy=cf.DATA03['vanishing_strategy'], act_output=None,
                              reg_lambda=None,
    ...
    for opt in cf.LIST_GD_MODELS:
        for seed in cf.LIST_SEEDS:
            md2 = GdAnfisClassifier(num_rules=cf.DATA03['n_rules'], mf_class=cf.DATA03['mf_class'],
                                    vanishing_strategy=cf.DATA03['vanishing_strategy'], act_output=None,
                                    reg_lambda=None,
    ...
    for opt in cf.LIST_BIO_MODELS:
        for seed in cf.LIST_SEEDS:
            md3 = BioAnfisClassifier(num_rules=cf.DATA03['n_rules'], mf_class=cf.DATA03['mf_class'],
                                     vanishing_strategy=cf.DATA03['vanishing_strategy'], act_output=None,
                                     reg_lambda=None,
    ...
    df_loss.to_csv(f"{cf.PATH_SAVE}/{cf.DATA03['name']}/{cf.FILE_LOSS}", index=False, header=True)
    df_result.to_csv(f"{cf.PATH_SAVE}/{cf.DATA03['name']}/{cf.FILE_RESULT}", index=False, header=True)

    # Get metrics statistics
    stat_df = df_result.groupby("model_name").agg(["mean", "std"])
    stat_df.to_csv(f"{cf.PATH_SAVE}/{cf.DATA03['name']}/{cf.FILE_RESULT_STATS}", index=True, header=True)

    # Draw boxplot metrics, path_save, figsize=(8, 6), exts=(".png", ), verbose=False
    draw_boxplot(df_result, data_name=cf.DATA03['name'], list_models=cf.LIST_MODEL_CC,
                 metrics=cf.LIST_METRIC_CLS, path_save=f"{cf.PATH_SAVE}/{cf.DATA03['name']}/bp",
                 figsize=(8, 6), exts=(".png", ".pdf"), verbose=False)

    # Draw convergence chart
    draw_convergence_chart(df_loss, data_name=cf.DATA03['name'], list_models=cf.LIST_MODEL_CC,
                           path_save=f"{cf.PATH_SAVE}/{cf.DATA03['name']}/cc",
                           figsize=(8, 6), exts=(".png", ".pdf"), verbose=False)

    print(f"Done with data: {cf.DATA03['name']}")
